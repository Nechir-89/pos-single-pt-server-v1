...
export const get_items_service = async (warehouse_id: number | null) => {
  console.log(`Getting items of warehouse ${warehouse_id}`);
  try {
    const query = `SELECT * FROM items WHERE warehouse_id = $<warehouse_id>`;
    const items: Item[] = warehouse_id ? await db.any(query, { warehouse_id }) : await db.any(query);
    return items;
  } catch (error) {

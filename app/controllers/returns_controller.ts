...
export const get_returns: RequestHandler<
  never,
  Response,
  never,
  never
> = async (req, res: Response) => {
  ...
  try {
    const respond = await get_returns_service();
...
export const get_return: RequestHandler<
  { id: string },
  Response,
  never,
  never
> = async (req, res: Response) => {
  try {
    const respond = await get_return_service(Number(req.params.id));
    res.status(200).json(respond);
  } catch (error) {
    console.log(`server running into an error: ${error}`)
    res.status(500).json({ error: `Server error` });
